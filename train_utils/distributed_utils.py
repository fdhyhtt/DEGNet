...
class MetricLogger(object):
    ...
    def log_every_(self, iterable, print_freq, header=None, i=0):
        # i = 0
        if not header:
            header = ""
        start_time = time.time()
        ...
        if len(iterable) > 1:
            d1, d2 = iterable
            space_fmt = ":" + str(len(str(len(d1)))) + "d"
        else:
            space_fmt = ":" + str(len(str(len(iterable)))) + "d"
        if torch.cuda.is_available():
            log_msg = self.delimiter.join([header,
        ...
        MB = 1024.0 * 1024.0
        if len(iterable) > 1:
            for obj in zip(d1, d2):
                data_time.update(time.time() - end)
                yield obj
                iter_time.update(time.time() - end)
                if i % print_freq == 0 or i == len(d1) - 1:
                    eta_second = iter_time.global_avg * (len(d1) - i)
                    eta_string = str(datetime.timedelta(seconds=eta_second))
                    if torch.cuda.is_available():
                        print(log_msg.format(i, len(d1),
                                             eta=eta_string,
                                             meters=str(self),
                                             time=str(iter_time),
                                             data=str(data_time),
...
def is_dist_avail_and_initialized():
    """Check if it supports distributed environments"""
